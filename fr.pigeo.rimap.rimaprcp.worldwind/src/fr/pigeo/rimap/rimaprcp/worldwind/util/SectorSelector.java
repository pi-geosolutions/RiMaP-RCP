 ...
 * enable/disable. Register a property listener to
 * receive changes to the sector as they occur, or just wait until the user is
 * done and then query the result via {@link
 * #getSector()}.
 *
 * @author tag
 * @version $Id$
...
public class SectorSelector extends WWObjectImpl
	...
	public final static String SECTOR_DRAWN_PROPERTY = "gov.nasa.worldwind.SectorSelector.Drawn";
	...
	public void stageChanged(RenderingEvent event) {
		if (!event.getStage()
				.equals(RenderingEvent.AFTER_BUFFER_SWAP))
			return;

		// We notify of changes during this rendering stage because the sector
		// is updated within the region shape's
		// render method.
	...
	protected void notifySectorChanged() {
		if (this.getShape()
				.hasSelection() && this.getSector() != null
				&& !this.getSector()
						.equals(this.previousSector)) {
			this.firePropertyChange(SECTOR_PROPERTY, this.previousSector, this.getShape()
					.getSector());
	...
	//
	// Mouse events are used to initiate and track initial drawing of the
	// region. When the selector is enabled it is
	...
	public void selected(SelectEvent event) {
		...
		if (this.getOperation() == NONE && event.getTopObject() != null && !(event.getTopPickedObject()
				.getParentLayer() == this.layer)) {
			this.setCursor(null);
			return;
		}

		if (event.getEventAction()
				.equals(SelectEvent.LEFT_PRESS)) {
			this.setPreviousPosition(this.getWwd()
					.getCurrentPosition());
		} else if (event.getEventAction()
				.equals(SelectEvent.DRAG)) {
			...
			this.setPreviousPosition(null);
				...
				.equals(SelectEvent.ROLLOVER) && this.getOperation() == NONE) {
			if (!(this.getWwd() instanceof Component))
				return;

			if (event.getTopObject() == null || event.getTopPickedObject()
					.isTerrain()) {
				this.setCursor(null);
				return;
			}
	...
	protected int determineAdjustmentSide(Movable dragObject, double factor) {
		if (dragObject instanceof SurfaceSector) {
			...
			double dN = abs(s.getMaxLatitude()
					.subtract(p.getLatitude()).degrees);
			double dS = abs(s.getMinLatitude()
					.subtract(p.getLatitude()).degrees);
			double dW = abs(s.getMinLongitude()
					.subtract(p.getLongitude()).degrees);
			double dE = abs(s.getMaxLongitude()
					.subtract(p.getLongitude()).degrees);
	...
	protected Sector resizeShape(Movable dragObject, int side) {
		if (dragObject instanceof SurfaceSector) {
			...
			Angle dLat = p.getLatitude()
					.subtract(this.getPreviousPosition()
							.getLatitude());
			Angle dLon = p.getLongitude()
					.subtract(this.getPreviousPosition()
							.getLongitude());
			...
			if (side == NORTH) {
				newMaxLat = s.getMaxLatitude()
						.add(dLat);
			} else if (side == SOUTH) {
				newMinLat = s.getMinLatitude()
						.add(dLat);
			} else if (side == EAST) {
				newMaxLon = s.getMaxLongitude()
						.add(dLon);
			} else if (side == WEST) {
				newMinLon = s.getMinLongitude()
						.add(dLon);
	...
	protected void dragWholeShape(DragSelectEvent dragEvent, Movable dragObject) {
		...
		EllipsoidalGlobe globe = (EllipsoidalGlobe) getWwd().getModel()
				.getGlobe();
		...
		double y = dragEvent.getMouseEvent()
				.getComponent()
				.getSize().height - screenRefPoint.y + dy - 1;
	...
	protected static class RegionShape extends SurfaceSector {
		...
		protected RegionShape(Sector sector) {
			...
			this.getBorder()
					.setHighlightAttributes(borderAttrs);
		}
		...
		public void setBorderColor(Color color) {
			ShapeAttributes attr = this.getBorder()
					.getAttributes();
		...
		public void render(DrawContext dc) {
			...
			if (this.getStartPosition() != null) {
				...
				this.setStartPosition(pos.getTerrainObject()
						.getPosition());
			}
		}

...
public class WmsNode extends AbstractNode implements ICheckableNode {
	...
	public void loadFromJson(JsonNode node) {
		...
		this.style = JsonUtils.parseString(node, "cls", LayerType.WMS.toString());
		...
		this.opacity = JsonUtils.parseDouble(node, "opacity", this.opacity);
		this.url = JsonUtils.parseString(node, "url", this.url);
		this.layers = JsonUtils.parseString(node, "layers", "");
		this.legendurl = JsonUtils.parseString(node, "legend", "");
		this.metadata_uuid = JsonUtils.parseString(node, "uuid", "");
		this.format = JsonUtils.parseString(node, "format", this.format);
		this.tiled = JsonUtils.parseBool(node, "TILED", this.tiled);
		this.queryable = JsonUtils.parseBool(node, "queryable", this.queryable);
		this.checked = JsonUtils.parseBool(node, "checked", this.checked);
		// polygon query params
		String pq_layer = JsonUtils.parseString(node, "pq_layer", null);
		if (pq_layer != null) {
			boolean btype = JsonUtils.parseBool(node, "mobile_process", false);
			int pq_bandnb = JsonUtils.parseInt(node, "pq_bandnb", 0);
			String pq_header = JsonUtils.parseString(node, "pq_header", "");
			int pq_round = JsonUtils.parseInt(node, "pq_round", 0);

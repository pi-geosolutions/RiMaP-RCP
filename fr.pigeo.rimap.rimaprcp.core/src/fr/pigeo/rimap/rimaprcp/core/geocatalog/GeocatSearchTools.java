...
public class GeocatSearchTools {
	...
	public GeocatSearchTools(IPreferencesService prefService) {
		...
		catalogVersion = prefService.getString(RimapConstants.RIMAP_DEFAULT_PREFERENCE_NODE,
				RimapConstants.CATALOG_VERSION_PREF_TAG,
				RimapConstants.CATALOG_VERSION_PREF_DEFAULT, null);
	}
	...
	public GeocatSearchResultSet search(String text) {
		...
		//String searchUrl = this.baseUrl+"srv/"+tsn.iso3_code+"/q?_content_type=json&facet.q=&fast=index&from=1&resultType=details&sortBy=relevance&sortOrder=&to=20&any="+text;
			...
			if (httpClient != null) {
				...
				try {
					...
					if (entity != null) {
						String json ;
						if (catalogIsPadre1()) {
							//then we will get results in XML and have to transform to json
							/*String xml = EntityUtils.toString(entity);
							JSONObject xmlJSONObj = XML.toJSONObject(xml);
							json = xmlJSONObj.toString(2);*/
							
							//
							//TODO: find a proper implementation that would output exactly the same way as in GN3
	...
	private String buildSearchURL(String text, int fromIndex, int toIndex, String sortBy, String sortOrder) {
		if (catalogIsPadre1()) {
			return this.baseUrl+getLocalizedSRVPathFragment()+"q?facet.q=&fast=index&sortBy="+sortBy+"&sortOrder="+sortOrder+"&from="+fromIndex+"&to="+toIndex+"&any="+text;
		} 

		//if (catalogVersion.equals("3") || catalogVersion.startsWith("3.")) {
		return this.baseUrl+getLocalizedSRVPathFragment()+"q?_content_type=json&facet.q=&fast=index&resultType=details&sortBy="+sortBy+"&sortOrder="+sortOrder+"&from="+fromIndex+"&to="+toIndex+"&any="+text;
	}
	...
	public static String getMetadataWebLink(GeocatMetadataEntity entity) {
		String link="http://ne-risk.pigeo.fr/geonetwork/srv/fre/md.viewer#/pigeo_simple_view/"+entity.get_geonet_info().getId();
		return link;
	}
